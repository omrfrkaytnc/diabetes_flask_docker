...
# Glucose
df['NEW_Glucose'] = pd.cut(x=df['Glucose'], bins=[-1, 139, 200], labels=["normal", "prediabetes"])

# Age
df.loc[(df['Age'] < 35), "NEW_Age"] = 'young'
df.loc[(df['Age'] >= 35) & (df['Age'] <= 55), "NEW_Age"] = 'middleage'
df.loc[(df['Age'] > 55), "NEW_Age"] = 'old'


# BMI
df['NEW_BMI'] = pd.cut(x=df['BMI'], bins=[-1, 18.5, 24.9, 29.9, 100],
...
def hyperparameter_optimization(X, y, cv=3, scoring="f1"):
